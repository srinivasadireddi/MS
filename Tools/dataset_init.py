...
if __name__ == '__main__':
    sys.path.append('..')
# import Tools.json_file_editer as JFE
import File_management.json_file_editer as JFE
from typing import TYPE_CHECKING
if TYPE_CHECKING:
    from Tools.job_manager import JobManager as JM  # alias juste pour les types
...
def make_ds_with_PO_data(data_dir: str,
    ...
    - Loads vertical grid data from the experiment vgrid file (*_atm_vgrid_ml.nc)
    - Merges grid data with time-varying data
    - No need to compute dzghalf as it's already in the grid file
    If only_2d is True, loads only 2D files and skips 3D/vgrid files.
    Parameters
    ----------
    data_dir : str
    ...
    selected_files = selected_files_2d + (selected_files_3d if not only_2d else [])
    ...
    vgrid_candidates = sorted(glob.glob(os.path.join(data_dir, "*_atm_vgrid_ml.nc")))
    vgrid_file = vgrid_candidates[0] if vgrid_candidates else None
    
    ds_vgrid = None
    if not only_2d and vgrid_file is not None:
        ...
        Log.add_to_log(f"(DI0024) Dimensions in vgrid: {dict(ds_vgrid.sizes)}")
        if 'dzghalf' not in ds_vgrid.variables and 'zghalf' in ds_vgrid.variables:
            ds_vgrid = add_dzghalf(ds_vgrid)
        if 'dzghalf' in ds_vgrid.variables and 'height_2' in ds_vgrid['dzghalf'].dims:
            ds_vgrid['dzghalf'] = ds_vgrid['dzghalf'].rename({'height_2': 'height'})
        elif 'dzghalf' not in ds_vgrid.variables:
            Log.add_to_log("(DI0025) Warning: dzghalf and zghalf are both missing from the vertical grid file")
    else:
        Log.add_to_log(f"(DI0024) Warning: Vertical grid file not found in {data_dir} with pattern *_atm_vgrid_ml.nc")
    ...
    # Merge with vertical grid data if available and not only_2d
    if ds_vgrid is not None:
        ...
        common_dims = set(ds_combined.sizes.keys()) & set(ds_vgrid.sizes.keys())
    ...
    if ds_hgrid is not None:
        Log.add_to_log("(DI0074) Merging with horizontal grid data (cell areas)")
        # Find common dimensions
        if 'ncells' in ds_combined.dims and 'cell' in ds_hgrid.dims:
            if ds_combined.sizes['ncells'] == ds_hgrid.sizes['cell']:
                # Rename 'cell' to 'ncells' to match
                ds_hgrid = ds_hgrid.rename({'cell': 'ncells'})
                Log.add_to_log(f"(DI0073) Renamed 'cell' dimension to 'ncells'")
                area_dims = ds_hgrid['cell_area'].dims
                common_dims_h = set(area_dims) & set(ds_combined.sizes.keys())
                Log.add_to_log(f"(DI0071) After renaming - common dimensions: {common_dims_h}")
            else:
                Log.add_to_log(f"(DI0072) Dimension size mismatch: ncells={ds_combined.sizes['ncells']}, cell={ds_hgrid.sizes['cell']}")
        ...
        try:
            ...
            if 'cell_area' in ds_hgrid.variables:
                ...
                Log.add_to_log("(DI0075) Successfully merged cell area data")
            else:
                Log.add_to_log("(DI0076) No cell_area variable found in horizontal grid")
        except Exception as e:
            Log.add_to_log(f"(DI0077) Warning: Could not merge horizontal grid data: {e}")
            Log.add_to_log("(DI0078) Continuing without cell area information")
    ...
    Log.add_to_log(f"(DI0035) Final dataset shape: {dict(ds_combined.sizes)}")
...
def ds_init(myJob: 'JM', chunks='auto', only_2d=False) -> xr.Dataset:
    """
...
def test_r02b09_grid_access():
    ...
    try:
        ...
        Log.add_to_log(f"(DI0070) ERROR: Could not open grid file: {e}")
        return False
